...
def _reconnect_relative_absolute_steps(
    ...
    """Reconnect the shared relative-action state cache after deserialization.

    Processor configs serialize the two pipelines independently, so the live
    reference from ``AbsoluteActionsProcessorStep`` to
    ``RelativeActionsProcessorStep`` cannot be stored in JSON. Reconnect it
    after loading a pretrained processor pair.
    """
