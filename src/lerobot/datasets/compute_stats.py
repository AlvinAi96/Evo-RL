...
def _get_valid_chunk_starts(episode_indices: np.ndarray, chunk_size: int) -> np.ndarray:
    """Return starts for chunks that remain inside a single episode."""
    if chunk_size <= 0:
    ...
    starts = np.arange(total_frames - chunk_size + 1, dtype=np.int64)
...
def _compute_relative_chunk_batch(
    ...
    """Compute flattened relative-action chunks for a batch of start indices."""
    ...
    reference_states = all_states[start_indices]
    mask_dim = len(relative_mask)
    chunks[:, :, :mask_dim] -= reference_states[:, None, :mask_dim] * relative_mask[None, None, :]
...
def compute_relative_action_stats(
    ...
    """Compute action normalization stats in relative-trajectory space.

    Every action in a chunk is expressed relative to the state at the start of
    that chunk. Dimensions selected by ``exclude_joints`` retain their original
    absolute values. This matches the targets produced by
    ``RelativeActionsProcessorStep`` during policy training.
    """
    from lerobot.processor.relative_action_processor import RelativeActionsProcessorStep

    if ACTION not in features or OBS_STATE not in features:
        raise ValueError(f"Relative action stats require both '{ACTION}' and '{OBS_STATE}' features")
    ...
    relative_mask = np.asarray(mask_step._build_mask(action_dim), dtype=np.float32)
    ...
    if all_actions.ndim != 2 or all_actions.shape[1] != action_dim:
        raise ValueError(f"Expected action data with shape (frames, {action_dim}), got {all_actions.shape}")
    if all_states.ndim != 2 or all_states.shape[1] < action_dim:
        raise ValueError(
            f"State dimension must be at least action dimension {action_dim}, got {all_states.shape}"
        )
    ...
    logging.info(
        "Computing relative action stats from %d chunks (chunk_size=%d, workers=%d)",
        len(valid_starts),
        chunk_size,
        effective_workers,
    )
    ...
    stats = running_stats.get_statistics()
    logging.info(
        "Relative action stats: chunks=%d frames=%d relative_dims=%d/%d excluded=%d",
        len(valid_starts),
        len(valid_starts) * chunk_size,
        int(relative_mask.sum()),
        len(relative_mask),
        int(len(relative_mask) - relative_mask.sum()),
    )
    return stats
